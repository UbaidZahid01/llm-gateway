"""
Independent, self-contained scenario: Bob calls the gateway directly with just
a provider, model, and his own provider key, sending a multi-turn chat (system
prompt + conversation history). Verifies the audit log recorded exactly that
request/response.
...
keys itself, so it forwards yours from the X-Provider-Key header:
    python tests/test_2_developer_bob.py
"""
...
def main() -> None:
    # Make a multi-turn call: system prompt + prior turn + new user message.
    payload = {
        "provider": "openai",
        "model": "gpt-4o-mini",
        "messages": [
            {"role": "system", "content": "You are a terse assistant. Answer in a single word."},
            {"role": "user", "content": "What continent is Paris in?"},
            {"role": "assistant", "content": "Europe."},
    ...
    print("\nPASS: Bob — multi-turn chat call logged correctly in the audit trail.")
